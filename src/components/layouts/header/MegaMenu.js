            ...
            <div
              ...
              className={`menuItems top-level ${
                item.subServices.length > 1
                  ? "has-clild-menus lg:col-span-3 md:col-span-2"
                  : ""
              }`}
            >
              {/* Top-Level Title */}
              ...
              <ul
                className={`subMenus ${
                  item.subServices.length > 1
                    ? "grid grid-cols-2 mid-level gap-6 multi-col"
                    : ""
                }`}
              >
                  ...
                  <li key={subIdx} className="min-w-0">
                    {subItem.heading && (
                      <h5 className="text-xl mb-2">
                        <Link
                          className="heading"
                          href={`/services/${slugify(item.title)}/${slugify(
                            subItem.heading
                          )}`}
                        >
                          {subItem.heading}
                    ...
                    <ul className="lastChildMenus space-y-2">
                      {subItem.subNavs?.map((nav, i) => {
                        const hasMidlevel =
                          subItem.heading && subItem.heading.trim !== "";

                        const href = hasMidlevel
                          ? `/services/${slugify(item?.title)}/${slugify(
                              subItem?.heading
                            )}/${slugify(nav.title)}`
                          : `/services/${slugify(item?.title)}/${slugify(
                              nav.title
                            ...
                            >
                              {typeof nav.icon === "string" ? (
                                <Image
                                  src={nav.icon}
                                  alt={nav.title}
                                  width={nav.icon.width || 48}
                                  height={nav.icon.height || 48}
